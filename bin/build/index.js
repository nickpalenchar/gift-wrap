...
function unpackFunction(absolutePath) {
  ...
  getCLIName()
    .then(name => {
      // ensure the cli name is not already in use
      if(fs.existsSync(`/usr/local/giftwrap/clis/${name}`)){
        say('\n*** cli name ' + chalk.bold(name) + ' already exists. Try again ***');
        delete args['--name'];
        return getCLIName();
      }
      builder(sourceFn, name)
    })
